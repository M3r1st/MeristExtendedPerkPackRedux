class X2Effect_EPPRegenMist extends X2Effect_EPPAdditionalSmokeEffect;

var int HealAmount;
var int MaxHealAmount;
var bool bApplyToBleedingOutUnits;
...
static function X2Effect RegenMistEffect(optional bool bSkipAbilityCheck)
{
    local X2Effect_EPPRegenMist         Effect;
    local X2Condition_AbilityProperty   AbilityCondition;
    local X2Condition_UnitProperty      UnitPropertyCondition;
    local X2Condition_UnitStatCheck     UnitStatCheckCondition;

    Effect = new class'X2Effect_EPPRegenMist';
    Effect.HealAmount = `GetConfigInt("F_RegenerativeMist_HealAmount");
    Effect.MaxHealAmount = `GetConfigInt("F_RegenerativeMist_MaxHealAmount");
    Effect.bApplyToBleedingOutUnits = `GetConfigBool("F_RegenerativeMist_bApplyToBleedingOutUnits");
    ...
    if (!bSkipAbilityCheck)
    {
        ...
        AbilityCondition.OwnerHasSoldierAbilities.AddItem(class'X2Effect_EPPApplyRegenMistToWorld'.default.RelevantAbilityName);
...
defaultproperties
{
    HealthRegeneratedName = F_RegenerativeMist_HealthRegenerated
    EffectTickedFn = RegenerationTicked

    EffectName = F_RegenerativeMist
    WorldEffectClass = class'X2Effect_EPPApplyRegenMistToWorld'
}